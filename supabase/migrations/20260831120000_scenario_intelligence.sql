-- =============================================================
-- Migration: Scenario Intelligence RPC
-- =============================================================
-- This migration creates the `get_scenario_intelligence` function,
-- which performs geospatial operations securely within the database.
--
-- By defining SECURITY INVOKER, the function runs with the privileges
-- of the authenticated user calling it, thus enforcing all existing
-- RLS policies automatically.
--
-- The function calculates spatial intersections between the selected
-- scenario's affected_area (Polygon) and other geographic entities.
-- =============================================================
...
DECLARE
    ...
    v_result json;
BEGIN
    -- 1. Get the scenario (Returns nothing if RLS denies access, though scenarios are public)
    ...
    -- Using ST_Intersects between the habitation Point and scenario Polygon
    ...
    -- Only hazards belonging to the affected habitations
    ...
    -- Only risk assessments for the relevant hazards
    ...
    -- Alerts intersecting the scenario area or without a specific location (system-wide)
    -- ST_Intersects works for Geometry vs Geography, we just cast location as geography
    ...
    -- Field devices deployed in the affected area
...
COMMENT ON FUNCTION get_scenario_intelligence IS 'Returns operational intelligence (habitations, hazards, risks, alerts, resources) for a specific scenario using spatial intersection.';
